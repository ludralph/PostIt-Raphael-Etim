...
describe('Message Form Component', () => {
  ...
  it(`should ensure the send message button is disabled if message content and
    priority is invalid`, () => {
    const wrapper = setup();
    wrapper.setState({ content: '', priority: '' });
  ...
  it(`should ensure the send message button is enabled if message content and
    priority is valid`, () => {
    const wrapper = setup();
    wrapper.setState({ content: 'hello', priority: 'Normal' });
    const sendButton = wrapper.find('Button');
    expect(sendButton.props().disabled).toBe(false);
  });

  it('should render the connected component', () => {
    const connectedComponent = shallow(<ConnectedMessageForm {...props} store={store}/>);
